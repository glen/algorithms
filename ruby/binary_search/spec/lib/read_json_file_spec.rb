...
describe 'ReadJsonFile' do
  ...
  describe 'generate_sample_input_file' do
    ...
    let(:sample_input) { JSON.pretty_generate({"input"=>[]}) }
  ...
  describe 'validate_entries_in_json' do
    it 'returns false if all input keys have values' do
      expect(test_object.input_file_not_filled?({'input' => [3, 4, 5]})).to be false
      expect(test_object.input_file_not_filled?({'input' => [3, 4, 5], 'another_input' => 'something'})).to be false
    end

    it 'returns true if any inout keys are not filled' do
      expect(test_object.input_file_not_filled?({'input' => []})).to be true
      expect(test_object.input_file_not_filled?({'input' => [3, 4, 5], 'another_input' => ''})).to be true
    end
  end
  ...
  describe 'read_input_file' do
    context 'file is present' do
      ...
      let(:valid_input_file_contents) { {'input' => [1, 2, 3, 4]} }
      let(:invalid_input_file_contents) { {'input' => []} }
